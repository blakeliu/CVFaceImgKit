...
class OpencvInfer:
    ...
    def prepare(self, device: str = "cpu"):
        ...
        self.out_names = self.output_order or self._model.getUnconnectedOutLayersNames()
    ...
    def run(self, input):
        ...
        try:
            ...
            raise OpencvDNNRunException(f"Opencv DNN run error: {e!s}")
        return net_out
...
def regsiter_opencv_backend(register: Registry):
    register(
        fn=OpencvInfer,
        name=OpencvInfer.__name__,
        namespace="engine_backend",
        type="opencv",
    )
